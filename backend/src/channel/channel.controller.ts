...
export class ChannelController {
  ...
  @ApiTags('Channel')
  @Patch(':idx')
  @ApiOperation({ summary: '채널 패스워드 수정', description: 'Update Channel Password API' })
  @ApiResponse({ status: 409, description: '중복 이름' })
  @ApiCreatedResponse({ type: UpdateChannelDto })
  @ApiBody({ type: CreateChannelDto })
  updatePwd(@Param('idx') idx: string, @GetMember() member: MemberInfoDto, @Body() updateChannelDto: UpdateChannelDto) {
    return this.channelService.updatePwd(+idx, member.intraId, updateChannelDto);
  }
  ...
  @ApiTags('Channel')
  @Post('/admin/:idx')
  @ApiOperation({ summary: 'idx 채널의 administrator 인지 확인', description: 'Is administrator' })
  @ApiBody({ type: MemberInfoDto })
  @ApiParam({ name: 'idx', example: '3', description: 'Channnel Idx'})
  ...
  @ApiTags('Channel')
  @Post('/admin/set/:idx')
  @ApiOperation({ summary: 'idx 채널의 administrator 저장', description: 'set administrator' })
  @ApiBody({ type: MemberInfoDto })
  @ApiParam({ name: 'idx', example: '3', description: 'Channnel Idx'})
  setAdmin(@GetMember() member: MemberInfoDto, @Param('idx') idx: string, @Body() channelUserDto: ChannelUserDto) {
  ...
  @ApiParam({ name: 'idx', example: '3', description: 'Channnel Idx'})
  kickChannel(@GetMember() member: MemberInfoDto, @Body() channelUserDto: ChannelUserDto, @Param('idx') idx: string) {
